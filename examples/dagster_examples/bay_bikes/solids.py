...
@solid(
    config={
        'weather_api_key': Field(
            String,
            description=('An API key generated by the dark sky')
        ),
        ...
        'csv_name': Field(
            String,
            description=('Path to store the csv at the end')
        )
    },
    required_resource_keys={'volume'}
)
def download_weather_report(context, date_file_name: str, latitude: float, longitude: float, base_url: str) -> str:
    weather_report = []
    with open(date_file_name, 'r') as fp:
        dates = json.load(fp)
    url_prefix = '/'.join([base_url, context.solid_config['weather_api_key'], '{0},{1}'.format(latitude, longitude)])
    for date in dates:
        url = url_prefix+',{}?exclude={}'.format(date, ','.join(context.solid_config['times_to_exclude']))
